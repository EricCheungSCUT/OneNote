...
-(UIButton *)doneButton {
    if (!_doneButton) {
        _doneButton = [[UIButton alloc] initWithFrame:CGRectMake(self.frame.size.width/2+5, self.frame.size.height-60, self.frame.size.width/2-10, 55)];
...
-(UIButton *)QQButton{
    if (!_QQButton) {
        _QQButton = [[UIButton alloc] initWithFrame:CGRectMake(5, self.frame.size.height-60, self.frame.size.width/2-10, 55)];
        [_QQButton setTintColor:[UIColor whiteColor]];
        [_QQButton setTitle:@"QQ Login" forState:UIControlStateNormal];
        [_QQButton.titleLabel setFont:[UIFont systemFontOfSize:18.0]];
        [_QQButton setBackgroundColor:[UIColor colorWithRed:0.129 green:0.588 blue:0.953 alpha:1.000]];
        [_QQButton addTarget:self.delegate action:@selector(onQQLoginButtonPressed) forControlEvents:UIControlEventTouchUpInside];
    }
    return _QQButton;
}
